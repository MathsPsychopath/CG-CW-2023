...
void Wireframe::drawWireframe(DrawingWindow& window, Camera& camera, std::vector<ModelTriangle> objects) {
	for (const ModelTriangle& object : objects) {
		CanvasPoint first = canvasIntersection(camera, object.vertices[0].position, 2.0, glm::mat3(1.0));
		CanvasPoint second = canvasIntersection(camera, object.vertices[1].position, 2.0, glm::mat3(1.0));
		CanvasPoint third = canvasIntersection(camera, object.vertices[2].position, 2.0, glm::mat3(1.0));
