...
//
//			RayTriangleIntersection intersection = Raytrace::getClosestValidIntersection(camera.cameraPosition, direction, objects.loadedTriangles);
...
//			BarycentricCoordinates barycentric = getGouraudBarycentric(intersection.intersectedTriangle, intersection.intersectionPoint);
//			
//			Colour finalColor = vertices[0].renderedColor * barycentric.A + vertices[1].renderedColor * barycentric.B + vertices[2].renderedColor * barycentric.C;
...
void preprocess(PolygonData& objects, glm::vec3 lightPosition, glm::vec3 cameraPosition, LightOptions& lighting, bool& hasParametersChanged) {
	...
	Colour globalAmbientColor(75, 50, 25);
	...
	for (auto& vertex : objects.loadedVertices) {
		...
		std::cout << vertex.renderedColor << std::endl;
	}
}
